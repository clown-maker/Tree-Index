...
class LinearLayer(nn.Module):
    def __init__(self,input_dim,out_dim,active_op='prelu',use_batch_norm=False):
        super(LinearLayer,self).__init__()
        #self.use_batch_norm=use_batch_norm
        self.register_buffer('use_batch_norm',torch.tensor(use_batch_norm))
...
class Network(nn.Module):
    def __init__(self,embed_dim=24,
        ...
        self.register_buffer('item_num',torch.tensor(item_num))
        #self.window_num=len(feature_groups)
        self.register_buffer('window_num',torch.tensor(len(feature_groups)))
        #self.feature_num=sum(feature_groups)
        self.register_buffer('feature_num',torch.tensor(sum(feature_groups)))
        #self.embed_dim=embed_dim
        self.register_buffer('embed_dim',torch.tensor(embed_dim))
        #self.item_node_share_embedding=item_node_share_embedding
        self.register_buffer('item_node_share_embedding',torch.tensor(item_node_share_embedding))
    ...
    def preference(self,batch_item_index,batch_labels):#len(batch_item_index)==len(batch_labels). two dimension matrix
        pre_linear_part_inputs = torch.zeros((len(batch_item_index) * self.feature_num, self.embed_dim),
                                             device=batch_item_index.device)

        effective_item_index=batch_item_index>=0#negative index means the absence of user behaviour
        effective_items=batch_item_index[effective_item_index]
        effective_labels=batch_labels.expand(batch_item_index.shape)[effective_item_index]
        if self.item_node_share_embedding:
        ...
        linear_part_input=torch.cat((weight_batch_sum, self.node_embedding(batch_labels.view(-1))), -1)
        return self.LinearPart(linear_part_input)
