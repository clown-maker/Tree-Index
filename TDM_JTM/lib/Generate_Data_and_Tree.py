...
def cut(_input,_train,_test,_validation,_number):
    user_behav = dict()
    user_ids = list()
    item_ids = set()
    ...
    validation_user_ids=user_ids[_number:2*_number]
    train_user_ids = user_ids[2*_number:]
...
def _read(raw_file,test_record_num):
    ...
    return behavior_dict, train_sample, test_sample,validation_sample
...
def _init_tree(train_sample, test_sample,validation_sample, stat=None,kv_file=None):
    #train_sample record user id, item id, cate_id, behavior_id, timestampe. key the liks 'USERID', value is a array.
    #test_sample is like train_sample
    ...
    class Item:
        def __init__(self, item_id, cat_id):
            ...
            self.cat_id = cat_id
            self.code = 0 # code is the node_id
    ...
    items = []
    item_id_set = set()
    for sample in [train_sample, test_sample,validation_sample]:
    ...
    ids = np.array([item.item_id for item in items],dtype=np.int32)
    codes = np.array([item.code for item in items],dtype=np.int32)
    #data = np.array([[] for i in range(len(ids))])##data is item embeddings
    builder = TreeBuilder()
    return builder.build(ids, codes, stat=stat, kv_file=kv_file)
